  ...
  Object.defineProperties(this, {
    ...
    install: {
      ...
      value: function (dir) {
        ...
        if (this.exists) {
          var missing = false;
          if (!fs.existsSync(path.join(config.installPath, this._exe))) {
            this.log.warn('The main executable is missing or cannot be found (' + path.join(config.installPath, this._exe) + ')');
            missing = true;
          }
          if (!fs.existsSync(path.join(config.installPath, this.id + '.xml'))) {
            this.log.warn('The primary configuration file is missing or cannot be found (' + path.join(config.installPath, this.id + '.xml') + ')');
            missing = true;
          }
          if (missing.length > 0) {
        ...
        var installPath = path.join(config.installPath);
        
        // If the output directory does not exist, create it.
        fs.exists(installPath, function (exists) {
          if (!exists) {
            fs.mkdirSync(installPath);
          }
          // Write the configuration file
          fs.writeFile(path.resolve(installPath, me.id + '.xml'), me._xml, function () {
            // Write the exe file
            winsw.createExe(me.id, installPath, function () {
              me.execute('"' + path.resolve(installPath, me._exe) + '" install', function () {
                sleep(2);
                me.emit('install');
              });
            });
    ...
    uninstall: {
      ...
      value: function (waitTime) {
        ...
        var uninstaller = function () {
          ...
          me.execute('"' + path.resolve(config.installPath, me._exe) + '" uninstall', function (error, stdout, stderr) {
            if (error) {
              ...
              var rm = function (file) {
                if (fs.existsSync(path.join(config.installPath, file))) {
                  fs.unlinkSync(path.join(config.installPath, file));
                }
              };
              ...
              var _other_files = fs.readdirSync(config.installPath).filter(function (file) {
              ...
              if (fs.readdirSync(config.installPath).length === 0) {
    ...
    exists: {
      ...
      get: function () {
        ...
        var installPath = path.resolve(config.installPath);
        return fs.existsSync(path.join(installPath, this.id + '.exe')) && fs.existsSync(path.join(installPath, this.id + '.xml'));
      }
    },
